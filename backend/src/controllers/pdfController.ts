...
async function convertOfficeToPdf(
  ...
  try {
    console.log(
      `Converting ${extension} to PDF using Native Office COM worker...`,
    );
    const pdfBytes = await runPythonWorker(
      "office-to-pdf",
      inputBuffer,
      extension,
      "pdf",
    ...
    console.error(
      `Native ${extension} to PDF conversion failed:`,
      err,
  ...
  if (extension === "xls" || extension === "xlsx") {
    try {
      console.log(
        "Preprocessing Excel file with Python worker to fit column width and wrap text...",
      );
      processedBuffer = await runPythonWorker(
        "preprocess-excel",
        inputBuffer,
        extension,
        "xlsx",
      ...
      console.error(
        "Failed to preprocess Excel file, falling back to original layout:",
        err,
      );
    }
  }
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "LibreOffice (soffice) not found in PATH or at C:\\Program Files\\LibreOffice\\program\\soffice.exe",
        );
      }
    }
    ...
    if (!fs.existsSync(tempOutputPath)) {
      throw new Error(
        "LibreOffice conversion failed: PDF file was not generated.",
      );
    }
...
async function runPythonWorkerAdvanced(
  ...
  try {
    ...
    const resolvedArgs = extraArgs.map((arg) =>
      arg.replace(/\[OUTPUT\]/g, tempOutputPath),
    );
...
async function runGhostscriptCompress(inputBuffer: Buffer): Promise<Buffer> {
  ...
  try {
    ...
    console.warn(
      "Ghostscript not found in PATH. Falling back to pdf-lib structural optimization.",
    );
...
export const splitPDF = async (req: Request, res: Response) => {
  try {
    const file = req.file;
    const { splitMode, ranges, mergeAll, fixedPages, selectedPages, pages } =
      req.body;
    ...
    // 1. Custom Range mode
    if (splitMode === "custom") {
      const parsedRanges = JSON.parse(ranges || "[]") as {
        start: number;
        end: number;
      }[];
      if (parsedRanges.length === 0) {
        return res
          .status(400)
          .json({ error: "Please specify at least one custom range." });
      }
      ...
      if (merge) {
        ...
        res.setHeader(
          "Content-Disposition",
          'attachment; filename="split_merged.pdf"',
        );
        ...
        res.setHeader(
          "Content-Disposition",
          'attachment; filename="split_ranges.zip"',
        );
    ...
    if (splitMode === "fixed") {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="split_fixed.zip"',
      );
      return res.send(zipBuffer);
    }
    ...
    if (splitMode === "extract_all") {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="extracted_all_pages.zip"',
      );
      return res.send(zipBuffer);
    }
    ...
    if (splitMode === "extract_select") {
      ...
      if (pageIndices.length === 0) {
        return res
          .status(400)
          .json({ error: "No valid pages selected for extraction." });
      }
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="extracted_selected_pages.zip"',
      );
      return res.send(zipBuffer);
    }
...
export const compressPDF = async (req: Request, res: Response) => {
  try {
    ...
    const result = await runPythonWorkerAdvanced(
      "compress",
      file.buffer,
      "pdf",
      "pdf",
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to compress PDF." });
  }
};
...
export const rotatePDF = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to rotate PDF." });
  }
};
...
export const watermarkPDF = async (req: Request, res: Response) => {
  try {
    ...
    for (const page of pages) {
      ...
      const contentWidth =
        type === "image" && embeddedImage
          ? embeddedImage.width * (watermarkSize / 100)
          : text.length * (watermarkSize * 0.6);
      const contentHeight =
        type === "image" && embeddedImage
          ? embeddedImage.height * (watermarkSize / 100)
          : watermarkSize;
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to add watermark to PDF." });
  }
};
...
export const addPageNumbers = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to add page numbers." });
  }
};
...
export const protectPDF = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to protect PDF." });
  }
};
...
export const unlockPDF = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(400)
      .json({ error: "Incorrect password or failed to decrypt PDF." });
  }
};
...
export const jpgToPDF = async (req: Request, res: Response) => {
  try {
    ...
    if (!files || files.length === 0) {
      return res
        .status(400)
        .json({ error: "Please upload at least one image file." });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="images_to.pdf"',
    );
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to convert images to PDF." });
  }
};
...
export const organizePDF = async (req: Request, res: Response) => {
  try {
    ...
    if (!pagesOrderStr) {
      return res
        .status(400)
        .json({ error: "Page order selection is required." });
    }
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to organize PDF." });
  }
};
...
export const signPDF = async (req: Request, res: Response) => {
  try {
    ...
    // Cache embedded images to avoid embedding the same signature multiple times
    const signatureImageCache: { [key: string]: any } = {};

    const getEmbeddedImage = async (dataUrl: string) => {
      if (signatureImageCache[dataUrl]) {
        return signatureImageCache[dataUrl];
      }
      ...
      signatureImageCache[dataUrl] = embeddedImg;
      return embeddedImg;
    };

    if (signaturesStr) {
      // Multiple signatures mode
      const signatures = JSON.parse(signaturesStr);
      for (const sig of signatures) {
        let pageIndices: number[] = [];
        if (sig.pages !== undefined) {
          pageIndices = parseRanges(String(sig.pages), totalPages);
        } else if (sig.pageIndex !== undefined) {
          const pIdx = parseInt(sig.pageIndex, 10);
        ...
        const embeddedImg = await getEmbeddedImage(sig.signature);
        const px = parseFloat(sig.x || "0");
        const py = parseFloat(sig.y || "0");
        const pw = parseFloat(sig.width || "150");
        const ph = parseFloat(sig.height || "75");
      ...
      // Fallback / Single signature mode
      if (!signatureData) {
      ...
      const embeddedImg = await getEmbeddedImage(signatureData);
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to sign PDF." });
  }
};
...
export const editPDF = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to edit PDF." });
  }
};
...
export const cropPDF = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to crop PDF." });
  }
};
...
export const redactPDF = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to redact PDF." });
  }
};
...
export const htmlToPDF = async (req: Request, res: Response) => {
  try {
    ...
    if (!content) {
      return res
        .status(400)
        .json({ error: "HTML source code or URL is required." });
    }
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to convert HTML to PDF." });
  }
};
...
export const repairPDF = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to repair PDF." });
  }
};
...
export const comparePDF = async (req: Request, res: Response) => {
  try {
    ...
    if (!files || files.length < 2) {
      return res
        .status(400)
        .json({ error: "Please upload two PDF files to compare." });
    }
    ...
    let report = {
      summary: { total_differences: 0, added: 0, removed: 0, modified: 0 },
      differences: [],
    };
    if (reportJsonBuffer) {
      try {
    ...
    return res.json({
      originalPdfBytes: originalPdfBuffer
        ? originalPdfBuffer.toString("base64")
        : "",
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to compare PDFs." });
  }
};
...
export const wordToPDF = async (req: Request, res: Response) => {
  try {
    ...
    if (!file) {
      return res
        .status(400)
        .json({ error: "Please upload a Word file (.docx)." });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="word_converted.pdf"',
    );
...
export const powerpointToPDF = async (req: Request, res: Response) => {
  try {
    ...
    if (!file) {
      return res
        .status(400)
        .json({ error: "Please upload a PowerPoint file (.pptx)." });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="powerpoint_converted.pdf"',
    );
    return res.send(pdfBytes);
  } catch (error: any) {
    console.error(
      "Error converting PowerPoint to PDF with LibreOffice:",
      error,
    );
    return res
      .status(500)
      .json({ error: error.message || "Failed to convert PowerPoint to PDF." });
  }
};
...
export const excelToPDF = async (req: Request, res: Response) => {
  try {
    ...
    const ext =
      path.extname(file.originalname).substring(1).toLowerCase() || "xlsx";
    const pdfBytes = await convertOfficeToPdf(file.buffer, ext);

    res.contentType("application/pdf");
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="excel_converted.pdf"',
    );
    return res.send(pdfBytes);
  } catch (error: any) {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to convert Excel to PDF." });
  }
};
...
export const pdfToWord = async (req: Request, res: Response) => {
  try {
    ...
    const docxBytes = await runPythonWorker(
      "pdf-to-docx",
      file.buffer,
      "pdf",
      "docx",
...
export const pdfToPowerpoint = async (req: Request, res: Response) => {
  try {
    ...
    const pptxBytes = await runPythonWorker(
      "pdf-to-pptx",
      file.buffer,
      "pdf",
      "pptx",
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="converted.pptx"',
    );
    return res.send(pptxBytes);
  } catch (error: any) {
    console.error(
      "Error converting PDF to PowerPoint with Python worker:",
      error,
    );
    return res
      .status(500)
      .json({ error: error.message || "Failed to convert PDF to PowerPoint." });
  }
};
...
export const pdfToExcel = async (req: Request, res: Response) => {
  try {
    ...
    const xlsxBytes = await runPythonWorker(
      "pdf-to-xlsx",
      file.buffer,
      "pdf",
      "xlsx",
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to convert PDF to Excel." });
  }
};
...
export const ocrPDF = async (req: Request, res: Response) => {
  try {
    ...
    if (!file) {
      return res
        .status(400)
        .json({ error: "Please upload a PDF or an Image file." });
    }

    const inputExt = file.originalname.split(".").pop() || "pdf";

    const outputBuffer = await runPythonWorker(
      "ocr",
      file.buffer,
      inputExt,
      "pdf",
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="ocr_searchable.pdf"',
    );
    return res.send(outputBuffer);
  } catch (error: any) {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to perform OCR." });
  }
};
...
export const detectForms = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to detect form fields." });
  }
};
...
export const saveForms = async (req: Request, res: Response) => {
  try {
    ...
    if (fieldsStr) {
      ...
      for (const f of fields) {
        ...
        if (pageIdx >= 0 && pageIdx < activePages.length) {
          ...
          const name =
            f.name ||
            `field_${pageIdx}_${Math.random().toString(36).substring(7)}`;
          ...
          if (isNew) {
            if (f.type === "text" || f.type === "date") {
              ...
              radioGroup.addOptionToPage(f.optionName || name, page, {
                x: px,
                y: py,
                width: pw,
                height: ph,
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="interactive_form.pdf"',
    );
    return res.send(Buffer.from(pdfBytes));
  } catch (error: any) {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to save form fields." });
  }
};
...
export const batchFillForms = async (req: Request, res: Response) => {
  try {
        ...
        try {
          ...
          if (field) {
            ...
            if (
              ...
              typeof (field as any).check === "function"
            ) {
              if (
              ...
              typeof (field as any).select === "function"
            ) {
          ...
          console.warn(
            `[Batch Fill] Missing or mismatched field: ${pdfFieldName}`,
            err,
          );
        }
      }
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="batch_filled_forms.zip"',
    );
    return res.send(zipBuffer);
  } catch (error: any) {
...
export const parseHeaders = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to parse data file headers." });
  }
};
