...
const PdfToExcel: React.FC<PdfToExcelProps> = ({ onBack }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Error converting PDF to Excel: ' + (err.response?.data?.error || err.message));
      setProgress(0);
    } finally {
    ...
    <div className="tool-page-container">
      <div className="tool-workspace">
        ...
        <div style={{ width: '100%', marginTop: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <h2 style={{ fontSize: '1.75rem', marginBottom: '1.5rem', fontFamily: 'var(--font-display)' }}>PDF to Excel</h2>
          ...
          {!downloadUrl ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>PDF Converted Successfully!</h3>
              <a href={downloadUrl} download="sheet.xlsx" className="btn btn-primary" style={{ textDecoration: 'none' }}>
          ...
          {isProcessing && (
            <ProgressBar progress={progress} message="Extracting text column structures and generating spreadsheet cell tables..." />
          )}
        </div>
      </div>
      ...
      <div className="tool-options-panel">
        ...
        <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
          Extracts tabular rows and columns from your PDF document and saves them as a Microsoft Excel spreadsheet (.xlsx).
        </p>
        ...
        >
          <FileSpreadsheet size={18} /> Convert PDF to Excel
        </button>
      </div>
    </div>
  );
