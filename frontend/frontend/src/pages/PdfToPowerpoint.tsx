...
const PdfToPowerpoint: React.FC<PdfToPowerpointProps> = ({ onBack }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Error converting PDF to PowerPoint: ' + (err.response?.data?.error || err.message));
      setProgress(0);
    } finally {
    ...
    <div className="tool-page-container">
      <div className="tool-workspace">
        ...
        <div style={{ width: '100%', marginTop: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <h2 style={{ fontSize: '1.75rem', marginBottom: '1.5rem', fontFamily: 'var(--font-display)' }}>PDF to PowerPoint</h2>
          ...
          {!downloadUrl ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <a href={downloadUrl} download="presentation.pptx" className="btn btn-primary" style={{ textDecoration: 'none' }}>
          ...
          {isProcessing && (
            <ProgressBar progress={progress} message="Generating slide vector maps and positioning layout layers..." />
          )}
        </div>
      </div>
      ...
      <div className="tool-options-panel">
        ...
        <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
          Transforms PDF pages into PowerPoint slide presentations (.pptx).
        </p>
        ...
        >
          <Presentation size={18} /> Convert PDF to PPT
        </button>
      </div>
    </div>
  );
