...
import { ArrowLeft, Lock, Download, FileCheck, Eye, EyeOff } from 'lucide-react';
...
const Protect: React.FC<ProtectProps> = ({ onBack }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Error protecting PDF: ' + (err.response?.data?.error || err.message));
      setProgress(0);
    } finally {
    ...
    <div className="tool-page-container">
      <div className="tool-workspace">
        ...
        <div style={{ width: '100%', marginTop: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <h2 style={{ fontSize: '1.75rem', marginBottom: '1.5rem', fontFamily: 'var(--font-display)' }}>Protect PDF File</h2>
          ...
          {!downloadUrl ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>PDF Encrypted and Protected!</h3>
              <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>Your password-protected PDF is ready to download.</p>
              <div style={{ display: 'flex', gap: '1rem', width: '100%', justifyContent: 'center', marginTop: '1.5rem', flexWrap: 'wrap' }}>
                ...
                <a 
                  href={downloadUrl} 
                  download={`protected_${files[0]?.name || 'document.pdf'}`}
          ...
          {isProcessing && (
            <ProgressBar progress={progress} message="Encrypting document and setting password..." />
          )}
        </div>
      </div>

      <div className="tool-options-panel">
        <h3 className="panel-title">Security Options</h3>
        <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
          Set a secure password to prevent unauthorized viewing of your PDF file. Anyone who attempts to open the document will be prompted for this password.
        </p>
