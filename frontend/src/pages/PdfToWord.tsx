...
const PdfToWord: React.FC<PdfToWordProps> = ({ onBack }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      alert('Error converting PDF to Word: ' + (err.response?.data?.error || err.message));
      setProgress(0);
    } finally {
    ...
    <div className="tool-page-container">
      <div className="tool-workspace">
        ...
        <div style={{ width: '100%', marginTop: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <h2 style={{ fontSize: '1.75rem', marginBottom: '1.5rem', fontFamily: 'var(--font-display)' }}>PDF to Word</h2>
          ...
          {!downloadUrl ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              ...
              <a href={downloadUrl} download="document.docx" className="btn btn-primary" style={{ textDecoration: 'none' }}>
          ...
          {isProcessing && (
            <ProgressBar progress={progress} message="Extracting characters and formatting editable paragraph flow..." />
          )}
        </div>
      </div>
      ...
      <div className="tool-options-panel">
        ...
        <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
          Extracts pages from your PDF document and formats them into an editable Microsoft Word document (.docx).
        </p>
        ...
        >
          <FileText size={18} /> Convert PDF to Word
        </button>
      </div>
    </div>
  );
