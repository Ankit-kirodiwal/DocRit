...
const ExtractPages: React.FC<ExtractPagesProps> = ({ onBack }) => {
  ...
  const handleSubmit = async () => {
    ...
    // Sort indices to extract in correct ascending order
    const extractIndices = pages
    ...
    try {
      ...
      setProgress(60);
      const copiedPages = await destDoc.copyPages(srcDoc, extractIndices);
    ...
    <div className="tool-page-container">
      <div className="tool-workspace">
        ...
        <div style={{ width: '100%', marginTop: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <h2 style={{ fontSize: '1.75rem', marginBottom: '1.5rem', fontFamily: 'var(--font-display)' }}>Extract PDF Pages</h2>
                  ...
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', maxWidth: '800px', marginBottom: '1.5rem' }}>
                      ...
                      <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', alignSelf: 'center' }}>
                        {selectedPages.size} pages selected for extraction
                      </span>
                    </div>
          ...
          {isProcessing && (
            <ProgressBar progress={progress} message="Extracting selected pages..." />
          )}
        </div>
      </div>

      <div className="tool-options-panel">
        <h3 className="panel-title">Extract Options</h3>
        <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
          Select the pages you want to extract from the PDF file, then click the button below to process.
        </p>
        <button 
          className="btn"
        ...
        >
          <Download size={18} /> Extract Pages ({selectedPages.size})
        </button>
      </div>
    </div>
  );
